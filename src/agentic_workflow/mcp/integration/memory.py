...
class MCPMemoryManager:
    ...
    async def initialize(self) -> None:
        ...
        for collection_name in self.mcp_collections.values():
            try:
                ...
                logger.debug(
                    f"Collection {collection_name} already exists or creation failed: {e}"
                )
    ...
    async def store_server_config(self, config: MCPServerConfig, agent_id: str) -> bool:
        ...
        try:
            memory_data = {
                ...
                "usage_count": 0,
                "success_rate": 0.0,
                "metadata": {"server_type": "mcp", "version": "1.0"},
            }
    ...
    async def store_capability(self, capability: MCPCapability, agent_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Stored capability {capability.name} from {capability.server_id}"
            )
            return True
    ...
    async def get_capabilities(
        self, server_id: Optional[str] = None, capability_type: Optional[str] = None
    ) -> List[MCPCapability]:
        """
        ...
        try:
            ...
            memory_results = await self.memory_manager.search_memories(
                collection=self.mcp_collections["capabilities"], tags=tags
            )
    ...
    async def record_execution(
        ...
        try:
            ...
            await self._update_performance_metrics(
                server_id, tool_name, success, execution_time
            )
    ...
    async def _update_performance_metrics(
        self, server_id: str, tool_name: str, success: bool, execution_time: float
    ) -> None:
        ...
        try:
            ...
            existing_metrics = await self.memory_manager.retrieve_memory(
                collection=self.mcp_collections["tool_performance"], memory_id=metric_id
            )
    ...
    async def _update_capability_usage(
        self, server_id: str, tool_name: str, success: bool
    ) -> None:
        """Update capability usage statistics."""
        try:
            ...
            capability_data = await self.memory_manager.retrieve_memory(
                collection=self.mcp_collections["capabilities"], memory_id=capability_id
            )

            if capability_data:
                capability_data["usage_count"] = (
                    capability_data.get("usage_count", 0) + 1
                )
                capability_data["last_used"] = datetime.now().isoformat()

                if success:
                    capability_data["success_count"] = (
                        capability_data.get("success_count", 0) + 1
                    )
                else:
                    capability_data["failure_count"] = (
                        capability_data.get("failure_count", 0) + 1
                    )
    ...
    async def get_execution_history(
        ...
        try:
            ...
            memory_results.sort(key=lambda x: x.get("timestamp", ""), reverse=True)
    ...
    async def get_performance_metrics(
        ...
        try:
            ...
            metrics = await self.memory_manager.search_memories(
                collection=self.mcp_collections["tool_performance"], tags=tags
            )
                ...
                if tool_name:
                    ...
                    if success:
                        successful_tools[tool_name] = (
                            successful_tools.get(tool_name, 0) + 1
                        )
    ...
    async def recommend_tools(
        self, agent_id: str, task_context: str, limit: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for capability in capabilities:
                ...
                # Preference score
                tool_prefs = preferences.get("preferred_tools", {}).get(
                    capability.name, {}
                )
                ...
                # Server reliability
                server_prefs = preferences.get("preferred_servers", {}).get(
                    capability.server_id, {}
                )
                    ...
                    recommendations.append(
                        {
                            ...
                            "context_relevance": (
                                len(common_words) if common_words else 0
                            ),
                        }
                    )
    ...
    async def store_learning_data(
        self, data_type: str, data: Dict[str, Any], agent_id: Optional[str] = None
    ) -> bool:
        """
            ...
            for collection_type, collection_name in self.mcp_collections.items():
                ...
                try:
                    memories = await self.memory_manager.search_memories(
                        collection=collection_name, tags=["mcp"]
                    )
    ...
    async def cleanup_old_data(self, retention_days: int = 30) -> bool:
        ...
        try:
            ...
            cleanup_collections = ["execution_history", "learning_data"]
                        ...
                        try:
                            ...
                            if memory_id:
                                await self.memory_manager.delete_memory(
                                    collection=collection_name, memory_id=memory_id
                                )
                                deleted_count += 1
                        except Exception as e:
                            logger.warning(
                                f"Failed to delete old memory {memory_id}: {e}"
                            )

                logger.info(
                    f"Cleaned up {deleted_count} old records from {collection_type}"
                )
