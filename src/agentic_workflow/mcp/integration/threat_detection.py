...
class ThreatDetectionEngine:
    ...
    async def analyze_connection_attempt(self, agent_id: str, server_id: str, 
                                        connection_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        """
        ...
        threat = await self._check_threat_indicators(agent_id, server_id, connection_data)
        if threat:
            threats.append(threat)
        
        # Analyze connection metadata
        threat = await self._analyze_connection_metadata(agent_id, server_id, connection_data)
        if threat:
            threats.append(threat)
    ...
    async def analyze_request(self, agent_id: str, server_id: str, tool_name: str,
        ...
        threat = await self._analyze_injection_patterns(agent_id, server_id, tool_name, request_data)
        if threat:
            threats.append(threat)
        
        # Check for data exfiltration attempts
        threat = await self._analyze_data_access_patterns(agent_id, server_id, tool_name, request_data)
        if threat:
            threats.append(threat)
        
        # Analyze parameter anomalies
        threat = await self._analyze_parameter_anomalies(agent_id, server_id, tool_name, request_data)
        if threat:
            threats.append(threat)
    ...
    async def analyze_response(self, agent_id: str, server_id: str, tool_name: str,
                             response_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        """
        ...
        threat = await self._analyze_sensitive_data_exposure(agent_id, server_id, tool_name, response_data)
        if threat:
            threats.append(threat)
        
        # Analyze response patterns for anomalies
        threat = await self._analyze_response_anomalies(agent_id, server_id, tool_name, response_data)
        if threat:
            threats.append(threat)
    ...
    async def _analyze_connection_patterns(self, agent_id: str, server_id: str) -> Optional[ThreatEvent]:
        ...
        if len(recent_requests) > 5:
            ...
            if len(set(round(i, 1) for i in intervals)) == 1:
                return ThreatEvent(
                    event_id=self._generate_event_id(agent_id, server_id, "automated_pattern"),
    ...
    async def _check_threat_indicators(self, agent_id: str, server_id: str, 
                                     connection_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        ...
        for key, indicator in self.threat_indicators.items():
            ...
            if check_value and re.search(pattern, str(check_value), re.IGNORECASE):
                ...
                return ThreatEvent(
                    event_id=self._generate_event_id(agent_id, server_id, "indicator_match"),
    ...
    async def _analyze_connection_metadata(self, agent_id: str, server_id: str,
                                         connection_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        ...
        if missing_fields:
            return ThreatEvent(
                event_id=self._generate_event_id(agent_id, server_id, "missing_metadata"),
        ...
        if "timestamp" in connection_data:
            ...
            if isinstance(timestamp, str):
                try:
                    ...
                    return ThreatEvent(
                        event_id=self._generate_event_id(agent_id, server_id, "invalid_timestamp"),
                        threat_type=ThreatType.MALFORMED_REQUEST,
            ...
            if timestamp > datetime.now() + timedelta(minutes=5):
                return ThreatEvent(
                    event_id=self._generate_event_id(agent_id, server_id, "future_timestamp"),
                    threat_type=ThreatType.SUSPICIOUS_BEHAVIOR,
                    threat_level=ThreatLevel.MEDIUM,
                    agent_id=agent_id,
                    server_id=server_id,
    ...
    async def _analyze_injection_patterns(self, agent_id: str, server_id: str, tool_name: str,
                                        request_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        ...
        for pattern_type, patterns in self.malicious_patterns.items():
            for pattern in patterns:
                if re.search(pattern, request_str, re.IGNORECASE):
                    ...
                    return ThreatEvent(
                        event_id=self._generate_event_id(agent_id, server_id, pattern_type),
    ...
    async def _analyze_data_access_patterns(self, agent_id: str, server_id: str, tool_name: str,
                                          request_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        ...
        suspicious_count = sum(1 for keyword in sensitive_keywords if keyword in request_str)
        
        if suspicious_count >= 3:
            return ThreatEvent(
                event_id=self._generate_event_id(agent_id, server_id, "sensitive_data_access"),
                threat_type=ThreatType.DATA_EXFILTRATION,
                threat_level=ThreatLevel.HIGH,
                agent_id=agent_id,
                server_id=server_id,
                ...
                evidence={"keyword_count": suspicious_count, "request_content": request_str},
                confidence=0.7
            )
        
        return None
    
    async def _analyze_parameter_anomalies(self, agent_id: str, server_id: str, tool_name: str,
                                         request_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        ...
        for key, value in request_data.items():
            if isinstance(value, str) and len(value) > 10000:
                return ThreatEvent(
                    event_id=self._generate_event_id(agent_id, server_id, "large_parameter"),
        ...
        for key in request_data.keys():
            for pattern in suspicious_param_patterns:
                if re.search(pattern, key):
                    return ThreatEvent(
                        event_id=self._generate_event_id(agent_id, server_id, "suspicious_parameter"),
                        threat_type=ThreatType.INJECTION_ATTACK,
                        threat_level=ThreatLevel.HIGH,
                        agent_id=agent_id,
                        server_id=server_id,
    ...
    async def _analyze_sensitive_data_exposure(self, agent_id: str, server_id: str, tool_name: str,
                                             response_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        ...
        for pattern in credential_patterns:
            if re.search(pattern, response_str):
                return ThreatEvent(
                    event_id=self._generate_event_id(agent_id, server_id, "data_exposure"),
                    threat_type=ThreatType.DATA_EXFILTRATION,
    ...
    async def _analyze_response_anomalies(self, agent_id: str, server_id: str, tool_name: str,
                                        response_data: Dict[str, Any]) -> Optional[ThreatEvent]:
    ...
    def add_threat_event(self, event: ThreatEvent) -> None:
        ...
        logger.warning(f"Threat detected: {event.description} (confidence: {event.confidence})")
    ...
    def get_threat_summary(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
        ...
        for event in events:
            ...
            # Count by agent
            summary["top_agents"][event.agent_id] = summary["top_agents"].get(event.agent_id, 0) + 1
            
            # Count by server
            if event.server_id:
                summary["top_servers"][event.server_id] = summary["top_servers"].get(event.server_id, 0) + 1
        ...
        summary["top_agents"] = dict(sorted(summary["top_agents"].items(), key=lambda x: x[1], reverse=True)[:10])
        summary["top_servers"] = dict(sorted(summary["top_servers"].items(), key=lambda x: x[1], reverse=True)[:10])
        
        return summary
    
    def get_agent_risk_score(self, agent_id: str, time_window: timedelta = timedelta(hours=24)) -> float:
        """Calculate risk score for an agent based on threat history."""
        cutoff = datetime.now() - time_window
        agent_events = [e for e in self.threat_events if e.agent_id == agent_id and e.timestamp > cutoff]
