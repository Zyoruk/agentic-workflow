...
class PromptResponseScanner:
    ...
    async def scan_prompt(
        ...
        if violations:
            logger.warning(
                f"Security violations found in prompt from {agent_id}: {len(violations)} violations"
            )
    ...
    async def scan_response(
        ...
        # Scan for compliance violations
        compliance_violations = await self._scan_compliance_patterns(
            response, "response"
        )
        violations.extend(compliance_violations)

        # Scan for suspicious patterns that might indicate compromise
        suspicious_violations = await self._scan_suspicious_response_patterns(
            response, "response"
        )
        ...
        if violations:
            logger.warning(
                f"Security violations found in response to {agent_id}: {len(violations)} violations"
            )

        return report

    async def _scan_injection_patterns(
    ...
    async def _scan_malicious_patterns(
        self, content: str, location: str
    ) -> List[SecurityViolation]:
    ...
    async def _scan_encoding_attacks(
        self, content: str, location: str
    ) -> List[SecurityViolation]:
        ...
        for encoding_type, patterns in self.encoding_patterns.items():
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    if self._is_legitimate_encoding(
                        match.group(), content, encoding_type
                    ):
                        continue
    ...
    async def _scan_compliance_patterns(
        self, content: str, location: str
    ) -> List[SecurityViolation]:
        """Scan for compliance violations."""
        violations = []
    ...
    async def _scan_suspicious_response_patterns(
        self, content: str, location: str
    ) -> List[SecurityViolation]:
    ...
    def _is_legitimate_encoding(
        self, match: str, content: str, encoding_type: str
    ) -> bool:
    ...
    def _determine_scan_result(
        self, risk_score: float, violations: List[SecurityViolation]
    ) -> SecurityScanResult:
        """Determine scan result based on risk score and violations."""
        # Check for critical violations that should be blocked immediately
        critical_types = {
            SecurityRiskType.INJECTION_ATTACK,
            SecurityRiskType.DATA_EXPOSURE,
        }
        critical_violations = [
            v for v in violations if v.risk_type in critical_types and v.severity >= 0.8
        ]
    ...
    async def _sanitize_content(
        self, content: str, violations: List[SecurityViolation]
    ) -> str:
        """Sanitize content by removing or masking violations."""
        sanitized = content

        for violation in violations:
            if violation.risk_type in {
                SecurityRiskType.INJECTION_ATTACK,
                SecurityRiskType.MALICIOUS_CONTENT,
            }:
    ...
    def get_scan_statistics(
        ...
        for report in reports:
            ...
            for violation in report.violations:
                ...
                stats["violation_types"][vtype] = (
                    stats["violation_types"].get(vtype, 0) + 1
                )
    ...
    def get_agent_security_profile(
