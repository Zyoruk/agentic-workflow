...
class Tool(ABC):
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_execution": self._executions[-1].start_time if self._executions else None
        }
...
class ToolRegistry:
    ...
    def list_tools(self, category: Optional[str] = None, tags: List[str] = None) -> List[str]:
...
class ToolDiscovery:
    ...
    def discover_from_module(self, module_name: str) -> List[str]:
        ...
        try:
            ...
            self.logger.info(f"Discovered {len(discovered_tools)} tools from {module_name}")
    ...
    def discover_from_package(self, package_name: str) -> List[str]:
        ...
        try:
            ...
            if hasattr(package, "__path__"):
                for finder, name, ispkg in importlib.iter_modules(package.__path__, package.__name__ + "."):
...
class ToolManager:
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info(f"Initialized tool manager with {len(built_in_tools)} built-in tools")
    ...
    async def execute_tool(
        ...
        if tool:
            return await self.registry.execute_tool(tool_identifier, inputs, agent_id, context)
        ...
        if matching_tools:
            ...
            self.logger.info(f"Using tool {best_match} as best match for '{tool_identifier}'")
            return await self.registry.execute_tool(best_match, inputs, agent_id, context)
        
        raise ValueError(f"No tool found matching '{tool_identifier}'")
    
    def recommend_tools(self, task_description: str, category: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        recommendations.sort(key=lambda x: (-x["success_rate"], x["avg_execution_time"]))
