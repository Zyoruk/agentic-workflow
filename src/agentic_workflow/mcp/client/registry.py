...
from .base import MCPClient, MCPServerConfig, MCPCapability
...
class MCPServerRegistry:
    ...
    async def setup_default_categories(self) -> None:
        ...
        default_categories = {
            'development': ServerCategory(
                ...
                servers=[
                    'github', 'git', 'vscode', 'jetbrains', 'code-analysis',
                    'build-tools', 'package-managers', 'debuggers'
                ],
                priority=1
            ),
            'data': ServerCategory(
                ...
                servers=[
                    ...
                    'elasticsearch', 'file-system', 'cloud-storage'
                ],
                priority=1
            ),
            ...
            'communication': ServerCategory(
                ...
                servers=[
                    'slack', 'teams', 'discord', 'email', 'jira',
            ...
            'security': ServerCategory(
                ...
                servers=[
                    'security-scanner', 'secret-manager', 'vulnerability-scanner',
                    'compliance-checker', 'audit-tools'
                ],
                priority=3
            ),
            ...
            'custom': ServerCategory(
                ...
                servers=[
                    'workflow-manager', 'code-intelligence', 'dev-environment',
                    'agent-communication', 'knowledge-manager', 'project-intelligence'
                ],
                priority=1
            )
        }
    ...
    async def register_server(self, config: MCPServerConfig, category: Optional[str] = None) -> bool:
        """
    ...
    async def connect_servers(self, category: Optional[str] = None, priority: Optional[int] = None) -> Dict[str, bool]:
        """
        ...
        if category:
            ...
            if category in self.categories:
                servers_to_connect = [
                    name for name in self.categories[category].servers
    ...
    async def create_server_preset(self, preset_name: str, server_names: List[str], description: str = "") -> bool:
        """
        ...
        try:
            ...
            missing_servers = [name for name in server_names if name not in self.server_configs]
            if missing_servers:
            ...
            logger.info(f"Created preset '{preset_name}' with {len(server_names)} servers")
            return True
    ...
    async def load_preset(self, preset_name: str) -> bool:
        ...
        logger.info(f"Loaded preset '{preset_name}': {success_count}/{total_count} servers connected")
