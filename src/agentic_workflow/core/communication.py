    ...
    async def send_message(self, message: Message) -> bool:
        ...
        try:
            if message.recipient_id:
                ...
                if len(self.messages[message.recipient_id]) >= self.max_messages_per_agent:
    ...
    async def receive_messages(self, agent_id: str) -> List[Message]:
        ...
        try:
            ...
            for msg in all_messages:
                if msg.expires_at:
                    expire_time = datetime.fromisoformat(msg.expires_at.replace('Z', '+00:00'))
...
class CommunicationManager:
    ...
    async def send_message(self, message: Message, channel_name: str = "default") -> bool:
    ...
    async def receive_messages(self, agent_id: str, channel_name: str = "default") -> List[Message]:
    ...
    async def broadcast_insight(self, insight_data: Dict[str, Any], channel_name: str = "default") -> bool:
    ...
    async def broadcast_message(self, message: Message, channel_name: str = "default") -> bool:
        """Broadcast a message to all agents."""
        if channel_name not in self.channels:
            self.logger.error(f"Unknown channel: {channel_name}")
            return False
    ...
    async def send_coordination_request(self, sender_id: str, task_id: str, 
                                      action_type: str, recipient_id: Optional[str] = None,
    ...
    async def send_notification(self, sender_id: str, notification_type: str,
    ...
    def get_message_history(self, agent_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        for channel_name, channel in self.channels.items():
            if isinstance(channel, InMemoryChannel):
                stats[f"{channel_name}_channel_stats"] = {
                    ...
                    "total_agent_messages": sum(len(msgs) for msgs in channel.messages.values()),
...
async def setup_agent_communication(agent_id: str, communication_manager: CommunicationManager,
