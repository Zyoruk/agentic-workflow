...
class TestMCPClient:
    ...
    async def test_register_server_success(
        self, mock_stdio_params, mock_session_class, sample_server_config
    ):
        ...
        # Register server
        with (
            patch.object(client, "_validate_server_command", return_value=True),
...
class TestMCPIntegration:
    ...
    async def test_full_workflow(self, sample_server_config, sample_capability):
        ...
        with (
            ...
            patch(
                "agentic_workflow.mcp.client.base.StdioServerParameters"
            ) as mock_stdio_params,
        ):
            ...
            with (
                patch.object(client, "_validate_server_command", return_value=True),
                patch.object(
                    client, "_connect_with_retries", return_value=mock_session
                ),
