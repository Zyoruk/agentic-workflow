...
class MCPEnhancedAgent(Agent):
    ...
    async def _connect_mcp_servers(self) -> None:
        ...
        connected_count = sum(1 for connected in self.connected_servers.values() if connected)
    ...
    async def _auto_discover_servers(self) -> None:
        ...
        try:
            ...
            logger.info(f"Agent {self.agent_id} discovered {len(discovered)} available servers")
            
            # Connect to high-priority discovered servers
            high_priority_servers = self.mcp_registry.get_servers_by_priority(1)
            for server_config in high_priority_servers[:5]:  # Limit to 5 auto-discovered
    ...
    async def _cache_mcp_capabilities(self) -> None:
        ...
        try:
            ...
            self.mcp_capabilities_cache['tools'] = [
                {'name': cap.name, 'description': cap.description, 'server': cap.server_id}
                for cap in tools
            ]
            ...
            self.dynamic_capabilities = [cap['name'] for cap in self.mcp_capabilities_cache.get('tools', [])]
            
            logger.info(f"Agent {self.agent_id} cached {len(tools)} tools, {len(resources)} resources, {len(prompts)} prompts")
            
        except Exception as e:
            logger.error(f"Failed to cache MCP capabilities for agent {self.agent_id}: {e}")
            ...
            if self.mcp_enabled and self.mcp_initialized:
                ...
                execution_steps.append({
                    ...
                    'data': {
                        ...
                        'connected_servers': len([s for s in self.connected_servers.values() if s])
                    }
                })
    ...
    async def plan(self, objective: str, context: Optional[Dict[str, Any]] = None) -> List[AgentTask]:
        """
    ...
    async def _assess_dynamic_capabilities(self, task: AgentTask) -> None:
        ...
        try:
            ...
            for tool_info in self.mcp_capabilities_cache.get('tools', []):
                if any(keyword in tool_info['description'].lower() 
            ...
            logger.debug(f"Agent {self.agent_id} identified {len(relevant_tools)} relevant tools for task")
            
        except Exception as e:
            logger.warning(f"Capability assessment failed for agent {self.agent_id}: {e}")
    ...
    def _assess_task_complexity(self, task: AgentTask) -> str:
        ...
        if any(keyword in description.lower() for keyword in ['complex', 'multiple', 'integrate', 'analyze']):
            return 'high'
        elif any(keyword in description.lower() for keyword in ['simple', 'basic', 'single']):
            return 'low'
        else:
            return 'medium'
    ...
    def _identify_required_capabilities(self, task: AgentTask) -> List[str]:
        ...
        for capability, keywords in capability_keywords.items():
            if any(keyword in description or keyword in task_type for keyword in keywords):
    ...
    def _select_optimal_tools(self, capability_matches: List[Dict], task_analysis: Dict) -> List[str]:
    ...
    async def _execute_with_mcp(self, task: AgentTask, reasoning_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not selected_tools:
                ...
                logger.info(f"No MCP tools selected, using traditional execution for agent {self.agent_id}")
                execution_result['execution_approach'] = 'traditional'
                execution_result['results']['message'] = 'Task executed using traditional approach'
            ...
            for tool_name in selected_tools:
                try:
                    if self.enhanced_tools:
                        ...
                        # Execute tool
                        tool_result = await self.enhanced_tools.execute_tool(tool_name, tool_params)
                        ...
                        logger.warning(f"Enhanced tools not available for agent {self.agent_id}")
                        
                except Exception as e:
                    logger.error(f"Tool execution failed for {tool_name} in agent {self.agent_id}: {e}")
                    execution_result['tools_executed'].append({
                        'tool': tool_name,
                        'success': False,
                        'error': str(e)
            ...
            if execution_result['success']:
                execution_result['final_result'] = self._combine_tool_results(execution_result['results'])
    ...
    def _extract_tool_parameters(self, task: AgentTask, tool_name: str) -> Dict[str, Any]:
    ...
    async def _update_capability_usage(self, task: AgentTask, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to update capability usage for agent {self.agent_id}: {e}")
    ...
    async def refresh_mcp_capabilities(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to refresh MCP capabilities for agent {self.agent_id}: {e}")
            return False
    ...
    async def connect_mcp_server(self, server_config: MCPServerConfig) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to connect MCP server {server_config.name} for agent {self.agent_id}: {e}")
            return False
...
def create_mcp_enhanced_agent(agent_class: Type[Agent], **mcp_kwargs) -> Type[MCPEnhancedAgent]:
    """
