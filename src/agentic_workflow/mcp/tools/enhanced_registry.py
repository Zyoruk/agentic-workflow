...
try:
    from agentic_workflow.tools import Tool as NewTool
    ...
    async def execute(self, **kwargs) -> Any:
        ...
        try:
            result = await self.mcp_client.execute_tool(
                self.capability.name, kwargs, self.capability.server_id
            )
...
class EnhancedToolRegistry:
    ...
    async def initialize(self) -> None:
        ...
        if self.mcp_client:
            ...
            self.mcp_client.add_event_callback(
                "capability_added", self._on_capability_added
            )
            self.mcp_client.add_event_callback(
                "capability_removed", self._on_capability_removed
            )
        ...
        logger.info(
            f"Enhanced tool registry initialized with {total_tools} tools across all systems"
        )
    ...
    async def _load_builtin_tools(self) -> None:
        ...
        builtin_tools = [
            ...
            },
            {
                ...
                "func": lambda expression, **kwargs: eval(
                    expression
                ),  # Note: eval is dangerous, use safer alternative in production
                "parameters": {
                    "expression": {
                        "type": "string",
    ...
    async def execute_tool(
        ...
        agent_id: str = "enhanced_registry",
    ) -> Any:
        """
        ...
        try:
            ...
            if self.new_tool_manager:
                try:
                    execution = await self.new_tool_manager.execute_tool(
                    ...
                    if execution.success:
                        ...
                        logger.warning(
                            f"New tool system execution failed for {tool_name}: {execution.error_message}"
                        )
                except Exception as e:
            ...
            execution_time = (datetime.now() - execution_start).total_seconds()
            await self._record_execution(
                tool_name, parameters, None, execution_time, False, str(e)
            )
            raise
    ...
    def get_tool_recommendations(
        ...
        for tool in self.get_all_tools().values():
            ...
            if any(
                word in metadata.description.lower() for word in context_lower.split()
            ):
                score += 10
    ...
    async def create_tool_workflow(
        self, workflow_name: str, tool_sequence: List[str], description: str = ""
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Created workflow '{workflow_name}' with {len(tool_sequence)} tools"
            )
            return True
    ...
    async def execute_workflow(
        self, workflow_name: str, initial_parameters: Dict[str, Any] = None
    ) -> List[Any]:
        """
        ...
        for tool_name in tool_sequence:
            try:
                ...
                logger.error(
                    f"Workflow {workflow_name} failed at tool {tool_name}: {e}"
                )
                raise
    ...
    def add_tool_alias(self, alias: str, tool_name: str) -> bool:
        ...
        if not self.get_tool(tool_name):
            logger.error(
                f"Cannot create alias '{alias}' for non-existent tool '{tool_name}'"
            )
            return False
    ...
    def get_execution_history(
        self, tool_name: Optional[str] = None, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def refresh_mcp_tools(self) -> None:
        ...
        logger.info(
            f"Refreshed MCP tools: removed {len(old_tools)}, added {len(self.mcp_tools)}"
        )
    ...
    async def close(self) -> None:
        ...
        if self.mcp_client:
            self.mcp_client.remove_event_callback(
                "capability_added", self._on_capability_added
            )
            self.mcp_client.remove_event_callback(
                "capability_removed", self._on_capability_removed
