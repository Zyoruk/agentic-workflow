...
class MQTTEventManager:
    ...
    async def connect(self) -> bool:
        ...
        if not MQTT_AVAILABLE:
            self.logger.warning(
                "MQTT client not available - event system will use local bus only"
            )
            return False
    ...
    async def subscribe_to_topic(
        self, topic: str, handler: Callable[[Event], None]
    ) -> None:
...
async def emit_agent_started(agent_id: str, agent_type: str, **kwargs) -> None:
    ...
    await emit_event(
        EventType.AGENT_STARTED, agent_id, {"agent_type": agent_type, **kwargs}
    )
...
async def emit_agent_stopped(agent_id: str, agent_type: str, **kwargs) -> None:
    ...
    await emit_event(
        EventType.AGENT_STOPPED, agent_id, {"agent_type": agent_type, **kwargs}
    )


# Task lifecycle events
async def emit_task_started(
    agent_id: str, task_id: str, task_type: str, **kwargs
) -> None:
...
async def emit_task_completed(
    agent_id: str, task_id: str, result: Dict[str, Any], **kwargs
) -> None:
...
async def emit_tool_executed(
    agent_id: str, tool_id: str, duration: float, success: bool, **kwargs
) -> None:
    """Emit tool execution event."""
    await emit_event(
        EventType.TOOL_EXECUTED,
        agent_id,
        {"tool_id": tool_id, "duration": duration, "success": success, **kwargs},
    )
