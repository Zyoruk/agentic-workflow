...
class MCPEnhancedAgentCreateRequest(BaseModel):
    ...
    mcp_servers: Optional[List[str]] = Field(
        default=None, description="MCP servers to connect to"
    )
    mcp_categories: Optional[List[str]] = Field(
        default=None, description="MCP server categories"
    )
    auto_discover_servers: bool = Field(
        default=False, description="Auto-discover available servers"
    )
    reasoning_enabled: bool = Field(
        default=True, description="Enable reasoning capabilities"
    )
    default_reasoning_pattern: str = Field(
        default="chain_of_thought", description="Default reasoning pattern"
    )
...
async def get_mcp_status() -> MCPSystemStatusResponse:
    ...
    try:
        ...
        connected_count = sum(
            1 for status in server_status.values() if status.get("connected", False)
        )

        # Count total capabilities
        total_capabilities = sum(
            status.get("capabilities_count", 0) for status in server_status.values()
        )
...
async def list_mcp_servers() -> List[MCPServerResponse]:
    ...
    try:
        ...
        servers = []
        for server_id, server_status in server_status.items():
...
async def list_mcp_capabilities(
    capability_type: Optional[str] = None, server_id: Optional[str] = None
...
async def execute_mcp_tool(
    ...
    try:
        ...
        result = await client.execute_tool(
            request.tool_name, request.parameters, request.server_id
        )

        execution_time = (datetime.now() - start_time).total_seconds()

        # Find which server was used
        capability = await client._find_capability(
            request.tool_name, "tool", request.server_id
        )
...
@router.post(
    "/agents",
    response_model=MCPEnhancedAgentResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_mcp_enhanced_agent(
    request: MCPEnhancedAgentCreateRequest,
) -> MCPEnhancedAgentResponse:
    ...
    try:
        ...
        return MCPEnhancedAgentResponse(
            ...
            connected_servers=len(
                [s for s in mcp_status["connected_servers"].values() if s]
            ),
...
async def refresh_mcp_capabilities(server_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Successfully refreshed MCP capabilities")
