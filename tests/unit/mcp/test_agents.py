...
from agentic_workflow.agents.base import AgentResult, AgentTask
from agentic_workflow.mcp.client.base import MCPServerConfig
from agentic_workflow.mcp.integration.agents import (
    MCPEnhancedAgent,
...
class TestMCPEnhancedAgent:
    ...
    @pytest.mark.asyncio
    async def test_mcp_initialization_success(
        self, mock_tools, mock_registry, mock_client
    ):
        ...
        mock_registry_instance.connect_servers = AsyncMock(
        ...
        agent = MCPEnhancedAgent(agent_id="test_agent", mcp_enabled=True)
    ...
    async def test_execute_without_mcp(self, sample_agent_task):
        ...
        agent = MCPEnhancedAgent(agent_id="test_agent", mcp_enabled=False)

        await agent.initialize()

        # Mock the parent execute method
        with patch.object(
            agent.__class__.__bases__[0], "execute", new_callable=AsyncMock
        ) as mock_execute:
    ...
    @patch("agentic_workflow.mcp.integration.agents.MCPServerRegistry")
    @patch("agentic_workflow.mcp.integration.agents.EnhancedToolRegistry")
    async def test_execute_with_mcp(
        self, mock_tools, mock_registry, mock_client, sample_agent_task
    ):
        ...
        with patch.object(
            agent, "_initialize_mcp", new_callable=AsyncMock
        ) as mock_init_mcp:
    ...
    async def test_task_complexity_assessment(self, sample_agent_task):
        ...
        complex_task = AgentTask(
            description="complex analysis with multiple integrations", type="analysis"
        )
    ...
    async def test_mcp_execution_with_tools(self, mock_tools, sample_agent_task):
        ...
        execution_result = await agent._execute_with_mcp(
            sample_agent_task, reasoning_result
        )
    ...
    async def test_mcp_execution_no_tools(self, sample_agent_task):
        ...
        reasoning_result = {"selected_tools": [], "approach": "traditional"}
    ...
    @patch("agentic_workflow.mcp.integration.agents.MCPServerRegistry")
    @patch("agentic_workflow.mcp.integration.agents.EnhancedToolRegistry")
    async def test_refresh_mcp_capabilities(
        self, mock_tools, mock_registry, mock_client
    ):
    ...
    async def test_connect_mcp_server(self, mock_registry, sample_server_config):
        ...
        mock_registry_instance.register_server.assert_called_once_with(
...
class TestCreateMCPEnhancedAgent:
    ...
    def test_create_enhanced_agent_class(self):
        ...
        EnhancedAgent = create_mcp_enhanced_agent(
            Agent, mcp_enabled=True, mcp_categories=["development"]
        )
    ...
    @patch("agentic_workflow.mcp.integration.agents.EnhancedToolRegistry")
    @pytest.mark.asyncio
    async def test_enhanced_agent_execution_fallback(
        self, mock_tools, mock_registry, mock_client, sample_agent_task
    ):
...
class TestMCPAgentIntegration:
    ...
    async def test_end_to_end_workflow(self, sample_agent_task, sample_server_config):
        ...
        with patch("agentic_workflow.mcp.integration.agents.MCPClient") as mock_client:
            with patch(
                "agentic_workflow.mcp.integration.agents.MCPServerRegistry"
            ) as mock_registry:
                with patch(
                    "agentic_workflow.mcp.integration.agents.EnhancedToolRegistry"
                ) as mock_tools:
                    ...
                    mock_registry_instance = AsyncMock()
                    mock_registry_instance.connect_servers.return_value = {
                        sample_server_config.name: True
                    }
                    mock_registry.return_value = mock_registry_instance
    ...
    async def test_error_handling_and_recovery(self, sample_agent_task):
        ...
        agent = MCPEnhancedAgent(agent_id="error_test_agent", mcp_enabled=True)

        # Test with various error conditions
        with patch("agentic_workflow.mcp.integration.agents.MCPClient") as mock_client:
            # Simulate intermittent failures
            mock_client_instance = AsyncMock()
            mock_client_instance.initialize.side_effect = [
...
class TestMCPEnhancedIntegration:
    ...
    async def test_reasoning_engine_initialization(self, mock_reasoning_engine):
        ...
        agent = MCPEnhancedAgent(
            agent_id="test_agent", mcp_enabled=False, reasoning_enabled=True
        )
    ...
    async def test_enhanced_reasoning_with_mcp_context(self):
        ...
        task = AgentTask(
            task_id="test_task", type="analysis", description="Analyze code repository"
        )
    ...
    async def test_reasoning_pattern_selection(self):
        ...
        agent = MCPEnhancedAgent(agent_id="test_agent", reasoning_enabled=True)
    ...
    async def test_backward_compatibility(self):
        ...
        agent_basic = MCPEnhancedAgent(
            agent_id="basic_agent", mcp_enabled=True, reasoning_enabled=False
        )
        ...
        agent_no_comm = MCPEnhancedAgent(
            agent_id="no_comm_agent", mcp_enabled=True, communication_manager=None
        )
        ...
        task = AgentTask(
            task_id="compat_test", type="basic", description="Basic compatibility test"
        )
