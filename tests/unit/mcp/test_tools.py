...
from agentic_workflow.mcp.client.base import MCPCapability, MCPClient
...
class TestEnhancedToolRegistry:
    ...
    async def test_unregister_builtin_tool(self, sample_builtin_tool):
        ...
        success = await registry.unregister_builtin_tool(
            sample_builtin_tool.metadata.name
        )
    ...
    async def test_tool_recommendations(self, sample_builtin_tool):
        ...
        assert (
            "echo" in recommended_names
            or sample_builtin_tool.metadata.name in recommended_names
        )
    ...
    async def test_tool_aliases(self, sample_builtin_tool):
        ...
        # Add alias
        success = registry.add_tool_alias(
            "echo_alias", sample_builtin_tool.metadata.name
        )
        assert success
    ...
    async def test_tool_workflows(self, sample_builtin_tool):
        ...
        success = await registry.create_tool_workflow(
            "test_workflow", ["echo", "current_time"], "Test workflow"
        )
        assert success

        # Execute workflow
        results = await registry.execute_workflow(
            "test_workflow", {"text": "workflow test"}
        )
    ...
    async def test_workflow_with_missing_tools(self):
        ...
        success = await registry.create_tool_workflow(
            "invalid_workflow", ["nonexistent_tool"], "Invalid workflow"
        )
        assert not success
    ...
    async def test_mcp_tool_integration(self, sample_mcp_capability, mock_mcp_client):
        ...
        result = await registry.execute_tool(
            sample_mcp_capability.name, {"input": "test"}
        )
...
class TestMCPTool:
    ...
    @pytest.mark.asyncio
    async def test_mcp_tool_usage_tracking(
        self, sample_mcp_capability, mock_mcp_client
    ):
