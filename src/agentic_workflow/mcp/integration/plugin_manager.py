...
class MCPServerPlugin(ABC):
    ...
    async def cleanup(self) -> None:
        ...
        self._initialized = False
        logger.info(f"Cleaned up plugin: {self.metadata.name if self.metadata else 'unknown'}")
...
class PluginManager:
    ...
    async def initialize(self) -> None:
        ...
        logger.info(f"Plugin manager initialized with {len(self.loaded_plugins)} plugins loaded")
    ...
    async def _load_plugin_file(self, plugin_file: Path) -> None:
        ...
        for name, obj in inspect.getmembers(module, inspect.isclass):
            if (issubclass(obj, MCPServerPlugin) and 
                obj is not MCPServerPlugin and 
                ...
                'plugins': [
                    {
                        ...
                        'server_config': config.server_config.__dict__ if config.server_config else None
                    }
    ...
    async def _load_plugin_instance(self, config: PluginConfig) -> None:
        ...
        if await plugin_instance.initialize():
            ...
            raise RuntimeError(f"Failed to initialize plugin instance {config.instance_name}")
    
    async def install_plugin(self, plugin_name: str, instance_name: str, 
                           config: Dict[str, Any], enabled: bool = True) -> bool:
        """
    ...
    async def get_all_server_configs(self) -> List[MCPServerConfig]:
        ...
        for instance_name, plugin in self.loaded_plugins.items():
            try:
                ...
                logger.error(f"Failed to get server config for plugin {instance_name}: {e}")
        ...
        for instance_name, config in self.plugin_configs.items():
            if config.enabled and instance_name in self.loaded_plugins:
                registry["loaded_plugins"][instance_name] = {
                    ...
                    "instance_name": config.instance_name,
                    "enabled": config.enabled,
                    "config_keys": list(config.config.keys()),  # Don't export sensitive config
                    "server_name": config.server_config.name if config.server_config else None
                }
