...
class TestTool:
    ...
    def test_execution_history(self):
        ...
        asyncio.run(self.mock_tool.execute_with_monitoring({"message": "test"}, "agent"))
    ...
    def test_performance_metrics(self):
        ...
        # Add some executions
        asyncio.run(self.mock_tool.execute_with_monitoring({"message": "test1"}, "agent"))
        asyncio.run(self.mock_tool.execute_with_monitoring({"message": "test2"}, "agent"))
...
class TestToolDiscovery:
    ...
    def test_discover_from_module(self):
        ...
        discovered = self.discovery.discover_from_module("agentic_workflow.tools.builtin")
...
class TestToolManager:
    ...
    def test_recommend_tools(self):
        ...
        dev_recommendations = self.manager.recommend_tools("text operations", category="development")
...
class TestBuiltinTools:
    ...
    async def test_data_analysis_tool(self):
        ...
        inputs = {
            "operation": "aggregate",
            "data": sales_data,
            "criteria": {"group_by": "region", "aggregate_key": "sales", "function": "sum"}
        }
        execution = await tool.execute_with_monitoring(inputs, "test_agent")
...
class TestToolIntegration:
    ...
    async def test_full_workflow(self):
        ...
        execution = await manager.execute_tool("text_processing_tool", inputs, "integration_test")
        
        assert execution.success
        assert execution.outputs["result"]["word_count"] == 2
        
        # Test recommendations
        recommendations = manager.recommend_tools("text processing", category="analysis")
        assert len(recommendations) > 0
