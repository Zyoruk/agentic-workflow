...
try:
    ...
    from mcp.client.models import Prompt, Resource, Tool
...
class MCPClient:
        ...
        async with self._lock:
            ...
            try:
                ...
                server_params = StdioServerParameters(
                    command=config.command, args=config.args or [], env=config.env or {}
                )
    ...
    async def _validate_server_command(self, config: MCPServerConfig) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["which", config.command[0]], capture_output=True, text=True, timeout=5
            )
    ...
    async def _connect_with_retries(
        self, config: MCPServerConfig, server_params
    ) -> ClientSession:
        ...
        for attempt in range(config.retry_attempts):
            try:
                ...
                if attempt < config.retry_attempts - 1:
                    ...
                    logger.warning(
                        f"Connection attempt {attempt + 1} failed for {config.name}, retrying in {wait_time}s: {e}"
                    )
                    await asyncio.sleep(wait_time)

        raise MCPConnectionError(
            f"Failed to connect after {config.retry_attempts} attempts: {last_error}"
        )
    ...
    async def _cache_server_capabilities(self, server_id: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Cached {len(capabilities)} capabilities for server {server_id}"
            )
        ...
        async def health_check_loop():
            ...
            while server_id in self.servers and config.auto_reconnect:
                ...
                if not await self._check_server_health(server_id):
                    logger.warning(
                        f"Server {server_id} health check failed, attempting reconnection"
                    )
    ...
    async def disconnect_server(
        self, server_id: str, cleanup_only: bool = False
    ) -> bool:
        """
    ...
    async def get_resource(
        self, resource_name: str, server_id: Optional[str] = None
    ) -> Any:
        """
    ...
    async def _find_capability(
        self, name: str, capability_type: str, server_id: Optional[str] = None
    ...
    async def list_capabilities(
        self, capability_type: Optional[str] = None, server_id: Optional[str] = None
    ) -> List[MCPCapability]:
        """
    ...
    def get_server_status(self, server_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if server_id:
            ...
            return {
                sid: self.get_server_status(sid) for sid in self.server_configs.keys()
            }
