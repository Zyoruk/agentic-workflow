    ...
    try:
        ...
        if store_result.success:
            ...
            retrieve_result = await memory_manager.retrieve(
                query="health check test", memory_type="SHORT_TERM", limit=1
            )

            healthy = retrieve_result.success and len(retrieve_result.results) > 0
            message = (
                "Memory system operational" if healthy else "Memory retrieval failed"
            )
        else:
            healthy = False
...
async def agent_registry_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        message = (
            f"Agent registry operational with {len(agent_types)} types"
            if healthy
            else "Agent registry issues detected"
        )
...
async def reasoning_engine_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            reasoning_works = (
                reasoning_result.completed
                and reasoning_result.confidence > 0
                and len(reasoning_result.steps) > 0
            )
        except Exception as e:
        ...
        message = (
            ...
            if healthy
            else "Reasoning engine issues detected"
        )
...
async def communication_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        message = (
            ...
            if healthy
            else "Communication system issues detected"
        )
...
async def tool_system_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        message = (
            f"Tool system operational with {len(available_tools)} tools"
            if healthy
            else "Tool system issues detected"
        )
...
async def configuration_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        message = (
            ...
            if healthy
            else "Configuration issues detected"
        )
    ...
    return {
        ...
        "summary": {
            ...
            "unhealthy_checks": sum(
                1 for r in results.values() if not r.get("healthy", False)
            ),
        },
    }
