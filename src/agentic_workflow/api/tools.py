...
class ToolSearchRequest(BaseModel):
    ...
    source: Optional[str] = Field(
        default=None, description="Tool source filter (builtin, mcp, new_system)"
    )
...
async def list_tools(
    source: Optional[str] = None, category: Optional[str] = None, limit: int = 100
) -> List[ToolResponse]:
    ...
    try:
        ...
        if category:
            tools = [
                tool for tool in tools if getattr(tool, "category", None) == category
            ]
...
async def execute_tool(request: ToolExecutionRequest) -> ToolExecutionResponse:
    ...
    try:
        ...
        result = await registry.execute_tool(
            request.tool_name, request.parameters, request.agent_id
        )
...
async def get_tool_performance_metrics() -> List[ToolPerformanceMetrics]:
    ...
    try:
        ...
        logger.info(
            f"Retrieved performance metrics for {len(performance_metrics)} tools"
        )
